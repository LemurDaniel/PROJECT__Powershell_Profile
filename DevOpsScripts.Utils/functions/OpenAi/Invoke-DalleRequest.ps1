        ...
        try {
          ...
          ([uri[]]$response.data.url) | 
                ForEach-Object -Begin {
                    ...
                    $extension = $_.LocalPath.split('/')[-1].split('.')[-1]
                    do {
                    ...
                    $_uri = $_
                    Invoke-WebRequest -Uri $_uri -OutFile $outFilePath
                    $returnObject += Get-Item -Path $outFilePath |
                    Select-Object -Property FullName, Length, LastWriteTime, @{Name = 'URL'; Expression = { $_uri } }
                }
            }
            else {
