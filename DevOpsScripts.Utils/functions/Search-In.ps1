...
function Search-In {
    ...
    BEGIN {
        ...
        $searchCollection = [System.Collections.ArrayList]::new()
    }
    PROCESS { 
        $null = $SearchObjects ?? @() | ForEach-Object { $searchCollection.Add($_) }
    }

    END {
        if ($SearchTags -eq '*') {
            $ChosenObjects = @{
                Property = $searchCollection | Get-Property -Property $returnProperty
            }
        }
        else {

            $ExcludeSearchTags = $ExcludeSearchTags ?? @()
            $ChosenObjects = $searchCollection | ForEach-Object {
