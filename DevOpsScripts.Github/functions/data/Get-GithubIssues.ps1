...
function Get-GithubIssues {
    ...
    $issues = Get-GithubCache -Identifier $Identifier -Account $repositoryData.Account

    if ($null -EQ $issues -OR $Refresh) {

        $releaseUrl = $repositoryData.issues_url -replace '{/number}', ''
        $issues = Invoke-GithubRest -Method GET -URL $releaseUrl -Account $repositoryData.Account
        $issues = $null -EQ $issues ? @() : $issues
        $issues = Set-GithubCache -Object $issues -Identifier $Identifier -Account $repositoryData.Account
    }
