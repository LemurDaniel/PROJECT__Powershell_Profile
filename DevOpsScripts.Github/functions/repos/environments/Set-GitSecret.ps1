<#
    .SYNOPSIS
    Adds or Updates a Secret to a Git-Repository via API.
    ...
    PS> Set-GitSecret -Environment dev -Name "Secret" -Value "SecretValue"

    .EXAMPLE

    Adding/Updating multiple secrets in a specific environment in the repository on the current path:

    PS> Set-GitSecret -Environment dev -Secrets @{
        Secret1 = "SecretValue"
        Secret2 = "SecretValue"
    }
...
function Set-GitSecret {
    ...
    $publicKey = Get-GitPublicKey -Environment $Environment -Account $Account -Context $Context -Repository $Repository
